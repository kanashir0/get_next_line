...
/*   Updated: 2024/12/07 11:34:06 by gyasuhir         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	static t_list	*lst_arr[1024];
	char			*line;

	if (fd < 0 || BUFFER_SIZE <= 0)
		return (NULL);
	generate_list(&lst_arr[fd], fd);
	if (&lst_arr[fd] == NULL)
		return (NULL);
	line = get_line_from_list(lst_arr[fd]);
	clear_list(&lst_arr[fd]);
	return (line);
}
